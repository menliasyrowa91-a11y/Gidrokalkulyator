...
export default function App() {
  ...
  const handleMessage = async (event) => {
    ...
    if (data === 'START_GPS') {
      ...
      if (status !== 'granted') { Alert.alert("Rugsat", "GPS rugsady hökman!"); return; }
      setPoints([]);
      Alert.alert("GPS", "Ölçeg başlandy! Nokat goşup başlaň.");
    } else if (data === 'ADD_POINT') {
      let loc = await Location.getCurrentPositionAsync({ accuracy: Location.Accuracy.High });
      setPoints(prev => [...prev, loc.coords]);
      Alert.alert("Nokat", `Goşuldy: ${points.length + 1} nokat`);
    } else if (data === 'CALC_GPS') {
      if (points.length < 3) { Alert.alert("Ýalňyşlyk", "Azyndan 3 nokat gerek!"); return; }
      let area = 0;
      for (let i = 0; i < points.length; i++) {
        let j = (i + 1) % points.length;
        area += (points[i].latitude * points[j].longitude) - (points[j].latitude * points[i].longitude);
      }
      const finalArea = Math.abs(area / 2) * 111320 * 111320 * Math.cos(points[0].latitude * Math.PI / 180);
      const ga = (finalArea / 10000).toFixed(4);
      Alert.alert("Netije", `Meýdan: ${ga} gektar`);
    }
  };
    ...
    <head>
        ...
        <style>
            * { box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif; }
            ...
            /* GPS stil */
            .gps-box { background: #dcfce7; padding: 15px; border-radius: 8px; margin-bottom: 20px; border: 1px solid #bbf7d0; }
        </style>
    </head>
    <body>
    <div class="wrapper">
        <div class="container">
            ...
            <div class="gps-box">
                <h3 style="margin-top:0; color:#065f46">GPS Meýdan Ölçeg</h3>
                <button style="background:#059669; margin: 2px 0;" onclick="window.ReactNativeWebView.postMessage('START_GPS')">Ölçäp Başla</button>
                <button style="background:#059669; margin: 2px 0;" onclick="window.ReactNativeWebView.postMessage('ADD_POINT')">Nokat Goş</button>
                <button style="background:#be123c; margin: 2px 0;" onclick="window.ReactNativeWebView.postMessage('CALC_GPS')">Meýdany Hasapla</button>
            </div>
            ...
            <div class="suwarysh-box">
                ...
                <div style="background: white; padding: 12px; border-radius: 8px; border: 1px solid #fcd34d; margin-bottom: 12px;">
                    <div style="font-weight: bold; color: #92400e; font-size: 13px; margin-bottom: 5px;">📊 1 GÜNDIKI SUWARÝAN MEÝDANY:</div>
                ...
                <div style="background: white; padding: 12px; border-radius: 8px; border: 1px solid #fcd34d;">
                    <div style="font-weight: bold; color: #92400e; font-size: 13px; margin-bottom: 5px;">⏳ WAGT TAPMAK:</div>
                    <div class="form-group" style="margin-top: 5px;">
                        <label style="font-weight: normal; font-size: 12px; color: #555;">Meýdan (Gektar):</label>
                        <input type="number" id="mel_S" value="10" step="1" inputmode="decimal" oninput="hasaplaMeliorasiya()">
                    </div>
                    <div style="font-weight: bold; color: #b45309; font-size: 14px; margin-top: 5px;" id="wagtNetije">0 sagat gerek.</div>
                </div>
            </div>
            <div class="author-credits">💻 Düzüji: Meňli Aşyrowa</div>
        </div>
    </div>
    <script>
        let mainTab = 'derya'; let yapType = 'acyk'; let häzirkiQ = 0;
        const tabelaF = {
        ...
        function switchMainTab(tab) { mainTab = tab; document.querySelectorAll('.tab').forEach(t => t.classList.remove('active')); document.getElementById('tab-' + tab).classList.add('active'); document.querySelectorAll('.section').forEach(s => s.classList.remove('active')); document.getElementById('sec-' + tab).classList.add('active'); häzirkiQ = 0; document.getElementById('netijeZonasy').style.display = 'none'; }
        function switchYapType(type) { yapType = type; document.getElementById('tab-acyk').classList.remove('active'); document.getElementById('tab-yapyk').classList.remove('active'); document.getElementById('tab-' + type).classList.add('active'); if(type === 'acyk') { document.getElementById('yap-acyk-fields').style.display = 'block'; document.getElementById('yap-yapyk-fields').style.display = 'none'; } else { document.getElementById('yap-acyk-fields').style.display = 'none'; document.getElementById('yap-yapyk-fields').style.display = 'block'; } häzirkiQ = 0; document.getElementById('netijeZonasy').style.display = 'none'; }
        function hasaplaÄhlisini() {
            let F = 0, V = 0, Q = 0;
            document.getElementById('res-meydan-row').style.display = 'flex'; document.getElementById('res-tizlik-row').style.display = 'flex';
            if (mainTab === 'derya') { let b_usti = parseFloat(document.getElementById('derya_b_usti').value); let b_asty = parseFloat(document.getElementById('derya_b_asty').value); let h = parseFloat(document.getElementById('derya_h').value); let L = parseFloat(document.getElementById('derya_L').value); let t = parseFloat(document.getElementById('derya_t').value); let hana = document.getElementById('derya_hana').value; if (isNaN(b_usti) || isNaN(b_asty) || isNaN(h) || isNaN(t) || t <= 0) { alert("Ölçegleri dolduryň!"); return; } let K = 0.82; if(hana === "dasly") K = 0.78; if(hana === "betonly") K = 0.88; F = ((b_usti + b_asty) / 2) * h; let V_yuz = L / t; V = V_yuz * K; Q = F * V; document.getElementById('resF').innerText = F.toFixed(2); document.getElementById('resV').innerText = V.toFixed(2); } else if (mainTab === 'yap' && yapType === 'acyk') { let b = parseFloat(document.getElementById('wodo_b').value); let H = parseFloat(document.getElementById('wodo_H').value); if (isNaN(b) || isNaN(H) || b <= 0 || H <= 0) { alert("Wodosliw ölçeglerini giriziň!"); return; } Q = 1.86 * b * Math.pow(H, 1.5); document.getElementById('res-meydan-row').style.display = 'none'; document.getElementById('res-tizlik-row').style.display = 'none'; } else if (mainTab === 'yap' && yapType === 'yapyk') { let diametr = document.getElementById('turba_d').value; let h = parseFloat(document.getElementById('turba_h').value); let v_input = parseFloat(document.getElementById('turba_V').value); if (isNaN(h) || h < 0 || isNaN(v_input)) { alert("Turbanyň çuňlygyny giriziň!"); return; } let golaýH = (Math.round(h * 10) / 10).toFixed(2); if (tabelaF[diametr] && tabelaF[diametr][golaýH] !== undefined) { F = tabelaF[diametr][golaýH]; } else { let jemi_asg_H = (diametr / 1000).toFixed(1); F = tabelaF[diametr][jemi_asg_H] || 0.1; } V = v_input; Q = F * V; document.getElementById('resF').innerText = F + " m² (Şezi)"; document.getElementById('resV').innerText = V.toFixed(2); }
            häzirkiQ = Q; let Q_litr = Q * 1000; document.getElementById('resQm3').innerText = Q.toFixed(3) + " m³/s"; document.getElementById('resQlitr').innerText = Math.round(Q_litr) + " l/s"; document.getElementById('netijeZonasy').style.display = 'block'; hasaplaMeliorasiya();
        }
        function hasaplaMeliorasiya() { if (häzirkiQ <= 0) return; let m = parseFloat(document.getElementById('mel_m').value); let yitgi = parseFloat(document.getElementById('mel_yitgi').value); let S_isleg = parseFloat(document.getElementById('mel_S').value); if (isNaN(m) || m <= 0) return; let Q_litr = häzirkiQ * 1000; let jemi_gektar_sutka = (Q_litr * yitgi * 86400) / (m * 1000); document.getElementById('meydanNetije').innerText = jemi_gektar_sutka.toFixed(2) + " gektar"; if (!isNaN(S_isleg) && S_isleg > 0) { let jemi_gundiz_wagt = (S_isleg * m) / (häzirkiQ * yitgi * 86400); let gundiz_bolum = Math.floor(jemi_gundiz_wagt); let galan_sagat = (jemi_gundiz_wagt - gundiz_bolum) * 24; let sagat_bolum = Math.floor(galan_sagat); let minut_bolum = Math.round((galan_sagat - sagat_bolum) * 60); let txt = ""; if(gundiz_bolum > 0) txt += gundiz_bolum + " gije-gündiz "; if(sagat_bolum > 0 || gundiz_bolum > 0) txt += sagat_bolum + " sagat "; txt += minut_bolum + " minut."; document.getElementById('wagtNetije').innerText = txt; } }
    </script>
    </body>
    </html>
  `;
